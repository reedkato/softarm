...
def win_build(self):
    ...
    #HiZボタン
    btn_frame = tk.Button(self.frame1,text="softHiZ",font=16,width=20,height=5,borderwidth=4,command=softhiz)
            ...
            for i in range(self.line):
                for j in range(3):
                    if (movedata[i][0] == "stop"):
                        ...
                        #mechadata[i][j] =  200 * movedata[i][j] / (20 * 3.14)
...
def setup():
    ...
    write(0x00,0x00,0x00) 
    #MIN_SPEED
    #write(0x08,0x08,0x08)
    #write(0x00,0x00,0x00) 
    #write(0x00,0x00,0x00) 
    #Kval_HOLD
    write(0x09,0x09,0x09)
    #write(0x49,0x49,0x49)
    #write(0x60,0x60,0x60)
    write(0x80,0x80,0x80)
    #write(0xF0,0xF0,0xF0)
    #Kval_RUN
    write(0x0A,0x0A,0x0A)
    write(0x9A,0x9A,0x9A)
    #write(0x80,0x80,0x80)
    ...
    #write(0xFF,0xFF,0xFF)
    #INT_SPEED
    write(0x0D,0x0D,0x0D)
    write(0x1E,0x1E,0x1E)
    write(0xD6,0xD6,0xD6)
    #ST_SLP
    write(0x0E,0x0E,0x0E)
    write(0x18,0x18,0x18)
    #FN_SLP_ACC
    write(0x0F,0x0F,0x0F)
    write(0x6C,0x6C,0x6C)
    #FN_SLP_DEC
    write(0x10,0x10,0x10)
    write(0x6C,0x6C,0x6C)
    #OCD_TH
    #write(0x13,0x13,0x13)
    #write(0x07,0x07,0x07)
    #STEP_MODE
    write(0x16,0x16,0x16)
    #write(0x00,0x00,0x00)
    write(0x07,0x07,0x07)
    #CONFIG
    write(0x18,0x18,0x18)
    #write(0xC0,0xC0,0xC0)
    #write(0x4C,0x4C,0x4C)
    #write(0x1E,0x1E,0x1E)
    write(0xDE,0xDE,0xDE)
    #write(0xCA,0xCA,0xCA)
    ##write(0xC2,0xC2,0xC2)
    write(0x88,0x88,0x88)
    #write(0xA8,0xA8,0xA8)
    #ResetPos(０点位置決め)
